	...
	const onConfirmPressed = async () => {
		...
		try {
			const response = await axios.put(
				`http://ec2-3-80-112-191.compute-1.amazonaws.com:8080/api/users/verify-new-user-code?email=${encodeURIComponent(
					email
			...
			if (response.data) {
				...
				const register_response = await axios.post(
					"http://ec2-3-80-112-191.compute-1.amazonaws.com:8080/api/users/register",
					{
						username: username,
						email: email,
						password: password,
